...
def train_classifier(model, train_loader, validation_loader, exp_name='experiment', lr=0.01, epochs=10, momentum=0.99, logdir='logs'):
    ...
    optimizer = Adam(model.parameters(), lr, betas=(0.9,momentum))
...
# %%
def build_df():
    train_stego_dir=['JMiPOD','JUNIWARD','UERD']
    record=[]
    feature_list={}
    for dir in train_stego_dir:
        ...
        for im in l:
            ...
            feature_list["class"]=1
            record.append(feature_list.copy())
    l=os.listdir('Cover')
    for im in l:
        feature_list["path"]=os.path.join("Cover",im)
        feature_list["class"]=0
        record.append(feature_list.copy())
...
def get_models():
    ...
    resnet18 = nn.Sequential(resnet18, nn.Linear(1000, 2), nn.Softmax(dim=1))
    models.append({'name': 'resnet18', 'model': resnet18, 'batch_size': 64, 'lr':0.1})

    resnet50=timm.create_model('resnet50', pretrained=True)
    resnet50 = nn.Sequential(resnet50, nn.Linear(1000, 2), nn.Softmax(dim=1))
    models.append({'name': 'resnet50', 'model': resnet50, 'batch_size': 16, 'lr':0.025})

    resnet101=timm.create_model('resnet101', pretrained=True)
    resnet101 = nn.Sequential(resnet101, nn.Linear(1000, 2), nn.Softmax(dim=1))
    models.append({'name': 'resnet101', 'model': resnet101, 'batch_size': 8, 'lr':0.0125})"""

    densenet121=timm.create_model('densenet121', pretrained=True)
    densenet121 = nn.Sequential(densenet121, nn.Linear(1000, 2), nn.Softmax(dim=1))
    models.append({'name': 'densenet121', 'model': densenet121, 'batch_size': 16, 'lr':0.025})

    maxvit_tiny=timm.create_model('maxvit_tiny_tf_512', pretrained=True)
    maxvit_tiny = nn.Sequential(maxvit_tiny, nn.Linear(1000, 2), nn.Softmax(dim=1))
    models.append({'name': 'maxvit_tiny__tf_512', 'model': maxvit_tiny, 'batch_size': 4, 'lr':0.00625})
